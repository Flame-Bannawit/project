      ...
      <div className="p-6 space-y-8 max-w-7xl mx-auto selection:bg-orange-500/30">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 animate-in fade-in duration-700">
          ...
          <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 p-2 rounded-[1.5rem] shadow-sm backdrop-blur-md">
            <div className="flex items-center gap-2 px-3">
              ...
              <input 
                type="date" 
                className="bg-transparent text-[10px] font-black uppercase outline-none text-slate-700 dark:text-white cursor-pointer"
            ...
            <button 
              onClick={fetchLogs}
              className="flex items-center gap-2 px-6 py-2.5 bg-orange-500 text-black font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-orange-400 transition-all active:scale-95 shadow-lg shadow-orange-500/20"
            >
        ...
        <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-xl dark:shadow-2xl transition-all overflow-x-auto">
          <table className="w-full text-left min-w-[800px]">
            <thead>
              <tr className="text-[9px] text-gray-500 font-black uppercase tracking-[0.2em] border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-black/20">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {loading ? (
                ...
                <tr><td colSpan={5} className="py-32 text-center text-gray-400 font-black uppercase text-[10px] tracking-[0.3em] italic">{lang === 'th' ? "ไม่พบข้อมูลในช่วงที่ระบุ" : "No records found in this range"}</td></tr>
              ) : logs.map((log) => (
                <tr key={log._id} className="hover:bg-slate-50 dark:hover:bg-white/[0.03] transition-colors group text-slate-700 dark:text-gray-300">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <div className="text-[9px] text-gray-400 font-bold uppercase mt-0.5 opacity-70">ID: {log._id.slice(-8)}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5">
                    ...
                    <div className="text-[8px] text-gray-400 font-black uppercase mt-1">KCAL</div>
                  </td>
                  <td className="px-8 py-5 uppercase">
                     <div className="text-[10px] font-black text-slate-700 dark:text-gray-300">{new Date(log.createdAt).toLocaleDateString(lang === 'th' ? 'th-TH' : 'en-GB')}</div>
                     <div className="text-[9px] font-bold text-gray-400">{new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                  </td>
                ...
                <div className="p-8 space-y-6">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-3 flex items-center gap-2">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-gray-400 font-black uppercase mt-1">{lang === 'th' ? "พลังงานทั้งหมด" : "Total Energy"}</div>
                    </div>
                  </div>
...
function MacroBadge({ label, val, color }: any) {
  return (
    <div className="bg-slate-100 dark:bg-white/5 px-2 py-1 rounded-lg border border-slate-200 dark:border-white/5 flex items-center gap-1.5 shadow-sm">
      <span className="text-[8px] font-black text-slate-400 dark:text-gray-600">{label}</span>
      <span className={`text-[10px] font-black ${color}`}>{Math.round(val || 0)}g</span>
    </div>
  );
}
...
function NutritionBox({ label, val, unit, color }: any) {
  return (
    <div className="bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/5 p-4 rounded-3xl text-center shadow-sm">
      <p className="text-[8px] font-black text-gray-400 dark:text-gray-600 uppercase mb-1 tracking-widest">{label}</p>
      <p className={`text-xl font-black ${color}`}>{Math.round(val || 0)}<span className="text-[10px] ml-0.5 opacity-50">{unit}</span></p>
    </div>
  );
}